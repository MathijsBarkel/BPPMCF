...
Graph graphConstructionCSP(const vector<vector<int>>& wq, int W, bool terminalZero) {
...
Solution solveAFCSPConcentration(const Instance& inst, const vector<vector<int>>& wq, int L2, int c, bool startWithReflect, int searchStrategy) {
	...
	// if a solution has been found within the time limit
		...
		if (sol.LB == sol.UB) {
			...
			// try to find a solution with more remaining space in one of the bins
			int minBins = sol.UB;
			...
			int fval, spaceGoal, mostSpace, totSpace = 0;
			vector<vector<vector<int>>> AByTail(inst.W + 1);
			bool first = true;
			int LBspace, UBspace;
			...
			if (printing) {
				...
				cout << "\n";
				cout << "Color " << c << ":\n";
				cout << "Minimum number of bins: " << sol.UB << ", total remaining space: " << totSpace << ", W-w_min = " << inst.W - wq[wq.size() - 1][0] << "\n";
