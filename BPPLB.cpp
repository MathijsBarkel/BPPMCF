...
Solution solveBPPLB(const Instance& inst, vector<int> &LBs, bool concentration, bool reflect) {
	...
	for (int c = 0; c < inst.C; c++) { 	// for each color
		...
		if (concentration && reflect) subsolutions[c] = solveAFCSPConcentration(inst, itemsSubset, inst.LBs[c], c, true, -1);
		else if (concentration && not reflect) subsolutions[c] = solveAFCSPConcentration(inst, itemsSubset, inst.LBs[c], c, false, -1);
