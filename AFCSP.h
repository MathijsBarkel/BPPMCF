...
Graph graphConstructionCSP(const vector<vector<int>>& wq, int W, bool terminalZero = false);
Solution solveAFCSP(const Instance& inst, const vector<vector<int>>& wq, int L2 = 0, int c = 0, int early = 0);
Solution solveAFCSPConcentration(const Instance& inst, const vector<vector<int>>& wq, int L2 = 0, int c = 0, bool startWithReflect= true, int searchStrategy = 0);
